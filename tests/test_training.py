...
def _fake_train_catboost_cv(X, y, X_test, categorical_features, cv, params, task_type, devices, **kwargs):
...
def _configure_training_test(tmp_path, monkeypatch) -> None:
    (tmp_path / "pyproject.toml").write_text("[project]\nname = 'test-project'\n")
    _write_competition_data(tmp_path)
    monkeypatch.setattr(training, "train_catboost_cv", _fake_train_catboost_cv)
...
def test_refined_profile_uses_isolated_outputs_and_iteration_override(tmp_path, monkeypatch) -> None:
    ...
    result = training.run_training(
        training.TrainingConfig(
            ...
            n_splits=2,
            profile="refined",
            run_name="refined-check",
            iterations=12,
        )
    )

    run_dir = tmp_path / "outputs" / "runs" / "refined-check"
    experiments = pd.read_csv(run_dir / "metrics" / "catboost_experiments.csv")
    assert result["profile"] == "refined"
    assert result["submission_path"] == run_dir / "submissions" / "catboost_submission.csv"
    assert {"deep_regularized_control", "deep_combined_features", "deep_depth_9"} <= set(
